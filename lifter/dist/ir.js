...
class IntermediateRepresentationLifter {
    ...
    getBlock(blockMap, microOp, offset) {
        const pc = microOp.args[0].value;
        const opc = microOp.args[1].value;
    ...
    replaceProxyJmps(blockMap, opcode) {
                ...
                block.opcodes.forEach((opcode) => {
                    if (opcode.microOp) {
                        if (opcode.microOp.instruction == "IF_BRANCH") {
                            ...
                            opcode.microOp.args[2].value.forEach((labeledMicroOp) => {
